...
@section('content')
    <div class="py-10 expenses-module">
        ...
        @if ($errors->updateExpense->any())
            <div class="alert alert-danger d-flex align-items-center p-5 mb-8">
                ...
                <div class="fw-semibold">No fue posible actualizar el gasto. Verifica los datos e inténtalo nuevamente.</div>
            </div>
        @endif
        ...
        <div class="d-flex flex-wrap justify-content-between align-items-center gap-4 mb-8">
            ...
            <button type="button" class="btn btn-primary fw-bold" data-bs-toggle="modal" data-bs-target="#createExpenseModal">
                <i class="ki-outline ki-plus fs-4 me-1"></i> Nuevo gasto
            </button>
        </div>
        ...
        <div class="card mb-8">
            <div class="card-header border-0 pt-6">
                <h3 class="card-title fw-bold">Configuración global de notificaciones</h3>
            </div>
            <div class="card-body pt-0">
                <form method="POST" action="{{ route('expenses.setup.global') }}" class="row g-5">
                    @csrf
                    @method('PUT')

                    <div class="col-md-3">
                        <label class="form-label required">Días previos de aviso</label>
                        <input type="number" name="days_before" min="0" max="365"
                            class="form-control @error('days_before', 'expenseGlobalSetup') is-invalid @enderror"
                            value="{{ old('days_before', (int) ($globalSetup->days_before ?? 0)) }}" required>
                    ...
                    <div class="col-md-4">
                        ...
                        <textarea name="emails" rows="2" class="form-control @error('emails', 'expenseGlobalSetup') is-invalid @enderror"
                            placeholder="correo1@dominio.com, correo2@dominio.com">{{ old('emails', implode(', ', (array) ($globalSetup->emails ?? []))) }}</textarea>
                        <div class="text-muted fs-8 mt-1">Separar con coma o salto de línea.</div>
                    ...
                    <div class="col-md-4">
                        ...
                        <textarea name="phones" rows="2" class="form-control @error('phones', 'expenseGlobalSetup') is-invalid @enderror"
                    ...
                    <table class="table table-row-bordered align-middle gy-5 mb-0">
                        ...
                        <tbody class="fw-semibold text-gray-700">
                            @forelse ($expenses as $expense)
                                <tr>
                                    <td>
                                        <div class="fw-bold text-gray-900">{{ $expense->concept }}</div>
                                    ...
                                    <td>
                                        <div class="fw-bold text-gray-900">{{ $expense->property?->internal_name ?? '-' }}</div>
                                        <div class="text-muted fs-7">{{ $expense->property?->internal_reference ?: '-' }}</div>
                                    </td>
                            ...
                            @empty
                                <tr>
                                    <td colspan="7" class="text-center py-16 text-muted">No hay gastos registrados.</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
                ...
                <form method="POST" action="{{ route('expenses.store') }}" enctype="multipart/form-data">
                    ...
                    <div class="modal-body">
                        @if ($errors->createExpense->any())
                            <div class="alert alert-danger mb-6">Revisa los datos del formulario.</div>
                        @endif

                        <div class="row g-5">
                            <div class="col-md-6">
                                <label class="form-label required">Propiedad</label>
                                <select name="property_id" class="form-select @error('property_id', 'createExpense') is-invalid @enderror" required>
                                    <option value="">Seleccionar...</option>
                                    @foreach ($properties as $property)
                                        <option value="{{ $property->id }}" {{ (string) old('property_id', $selectedProperty?->id) === (string) $property->id ? 'selected' : '' }}>
                                            {{ $property->internal_name }}{{ $property->internal_reference ? ' - ' . $property->internal_reference : '' }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="col-md-4">
                                ...
                                <div class="text-muted fs-8 mt-1">JPG, PNG, WEBP, PDF.</div>
    ...
    @foreach ($expenses as $expense)
        <div class="modal fade" id="editExpenseModal-{{ $expense->uuid }}" tabindex="-1" aria-hidden="true">
            <div class="modal-dialog modal-lg modal-dialog-scrollable">
                <div class="modal-content">
                        ...
                        <div class="modal-body">
                            <div class="row g-5">
                                ...
                                <div class="col-12">
                                    ...
                                    <input type="file" name="files[]" multiple accept=".jpg,.jpeg,.png,.webp,.pdf" class="form-control">
                                </div>
                                            ...
                                            @foreach ($expense->files as $file)
                                                <div class="d-flex align-items-center justify-content-between border rounded px-3 py-2">
                                                    <div class="d-flex align-items-center gap-3">
                                                        ...
                                                        @else
                                                            <span class="badge badge-light-primary text-primary">PDF</span>
                                                        @endif

                                                        <a href="{{ \Illuminate\Support\Facades\Storage::url($file->path) }}" target="_blank">
