        ...
        <div class="card">
            ...
            <div class="card-body pt-0 d-flex flex-column gap-6">
                            ...
                            @if ($document->file_path)
                                <div class="d-flex gap-2">
                                    ...
                                    @if ($canDeleteDossierFiles)
                                        <form method="POST" action="{{ route('dossiers.properties.documents.destroy', [$property, $document->document_type]) }}" onsubmit="return confirm('¿Eliminar este documento y su historial de versiones?');">
                                            @csrf
                                            @method('DELETE')
                                            ...
                                            <tr>
                                                ...
                                                <td class="text-end">
                                                    <div class="d-inline-flex gap-2">
                                                        ...
                                                        @if ($canDeleteDossierFiles)
                                                            <form method="POST" action="{{ route('dossiers.properties.documents.versions.destroy', [$property, $document->document_type, $version]) }}" onsubmit="return confirm('¿Eliminar esta versión del expediente?');">
                                                                @csrf
                                                                @method('DELETE')
                ...
                <div class="border rounded p-6">
                    ...
                    <form method="POST" action="{{ route('dossiers.properties.documents.store', $property) }}" enctype="multipart/form-data"
                        class="row g-4 align-items-end">
                        @csrf
                        <div class="col-lg-5">
                            <label class="form-label required">Nombre del documento</label>
                            <input type="text" name="label" class="form-control" placeholder="Ej: Convenio adicional"
                                value="{{ old('label') }}">
                        </div>
                        <div class="col-lg-4">
                            <label class="form-label">Vence el (opcional)</label>
