...
@section('content')
    <div class="py-10 property-module">
        <div class="mb-8">
            <a href="{{ route('tenants.edit', $tenant) }}" class="text-gray-600 text-hover-primary fw-semibold">
                <i class="ki-outline ki-arrow-left fs-4 me-1"></i> Volver a inquilino
            </a>
        </div>
        ...
        <div class="card mb-8">
            <div class="card-body p-8">
                <h1 class="mb-1 fw-bold">Expediente de inquilino</h1>
                <div class="fs-4 fw-bold text-gray-900">{{ $tenant->full_name }}</div>
                <div class="text-muted">{{ $tenant->phone_primary }} {{ $tenant->email ? '| ' . $tenant->email : '' }}</div>
            </div>
        </div>
        ...
        <div class="card">
            ...
            <div class="card-body pt-0 d-flex flex-column gap-6">
                            ...
                            @if ($document->file_path)
                                <div class="d-flex gap-2">
                                    ...
                                    @if ($canDeleteDossierFiles)
                                        <form method="POST" action="{{ route('dossiers.tenants.documents.destroy', [$tenant, $document->document_type]) }}" onsubmit="return confirm('¿Eliminar este documento y su historial de versiones?');">
                                            @csrf
                                            @method('DELETE')
                                            ...
                                            <tr>
                                                ...
                                                <td class="text-end">
                                                    <div class="d-inline-flex gap-2">
                                                        ...
                                                        @if ($canDeleteDossierFiles)
                                                            <form method="POST" action="{{ route('dossiers.tenants.documents.versions.destroy', [$tenant, $document->document_type, $version]) }}" onsubmit="return confirm('¿Eliminar esta versión del expediente?');">
                                                                @csrf
                                                                @method('DELETE')
                ...
                <h4 class="fw-bold mb-0">Otros documentos</h4>
                <div class="border rounded p-6">
                    ...
                    <form method="POST" action="{{ route('dossiers.tenants.documents.store', $tenant) }}" enctype="multipart/form-data"
                        ...
                        <div class="col-lg-5">
                            ...
                            <input type="text" name="label" class="form-control" placeholder="Ej: Aval adicional"
                                value="{{ old('label') }}">
                        </div>
                        <div class="col-lg-4">
                            <label class="form-label">Vence el (opcional)</label>
                        ...
                        <div class="col-12 d-flex justify-content-end">
                            <button type="submit" class="btn btn-primary">Agregar al expediente</button>
                        </div>
                    </form>
                </div>
                ...
                @if ($customDocuments->isNotEmpty())
                    ...
                    @endforeach
                @else
                    <div class="text-muted">Aun no hay documentos personalizados.</div>
                @endif
            </div>
        </div>
    </div>
